...
static int fido2_is_cred_in_specific_token(
        ...
        r = verify_features(d, path, require_fido2 ? LOG_ERR : LOG_DEBUG, NULL, NULL, &has_up, &has_uv);
...
static int fido2_generate_credential(
        ...
        r = verify_features(d, device, !!required_extensions ? LOG_ERR : LOG_DEBUG, &has_rk, &has_client_pin, &has_up, &has_uv);
...
int fido2_list_devices(void) {
        ...
        t = table_new("path", "manufacturer", "product", "hmac-secret", "rk", "clientpin", "up", "uv");
        if (!t) {
                r = log_oom();
                goto finish;
        }
