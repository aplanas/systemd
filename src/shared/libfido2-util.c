...
DLSYM_PROTOTYPE(fido_assert_set_clientdata) = NULL;
...
static int verify_features(
        ...
        if (require_hmac_secret && !has_hmac_secret)
...
static int fido2_assert_set_basic_properties(
                fido_assert_t *a,
                const char *rp_id,
                const uint8_t *cd, size_t cd_size,
                const void *cid,
                size_t cid_size) {
        int r;
        ...
        if (cd)
                r = sym_fido_assert_set_clientdata(a, cd, cd_size);
        else
                r = sym_fido_assert_set_clientdata_hash(a, (const unsigned char[32]) {}, 32);
        if (r != FIDO_OK)
                return log_error_errno(SYNTHETIC_ERRNO(EIO),
                                       "Failed to set FIDO2 assertion client data: %s", sym_fido_strerr(r));
...
static int fido2_is_cred_in_specific_token(
        ...
        r = verify_features(d, path, require_fido2 ? LOG_ERR : LOG_DEBUG, /* require_hmac_secret= */ true, /* ret_has_rk= */ NULL, /* ret_has_client_pin= */ NULL, &has_up, &has_uv, /* ret_has_always_uv= */ NULL);
        ...
        r = fido2_assert_set_basic_properties(a, rp_id, NULL, 0, cid, cid_size);
        if (r < 0)
                return r;
...
static int fido2_use_hmac_hash_specific_token(
        ...
        // TODO - Use fido2_generate_assert_specific_token to get the assert and extract the hmac-secret
        d = sym_fido_dev_new();
        if (!d)
                return log_oom();
...
int fido2_generate_credential(
        ...
        assert((required_extensions & ~FIDO2EXTENSION_HMAC_SECRET) == 0);
        ...
        r = verify_features(d, device, !!required_extensions ? LOG_ERR : LOG_DEBUG, FLAGS_SET(required_extensions, FIDO2EXTENSION_HMAC_SECRET), &has_rk, &has_client_pin, &has_up, &has_uv, &has_always_uv);
        if (r < 0)
                return r;
        ...
        int extensions = FLAGS_SET(required_extensions, FIDO2EXTENSION_HMAC_SECRET) ? FIDO_EXT_HMAC_SECRET : 0;
...
static int fido2_generate_assert_specific_token(
                const char *device,
                const char *rp_id,
                const void *salt,
                size_t salt_size,
        ...
        assert((lock_with & ~(FIDO2ENROLL_PIN|FIDO2ENROLL_UP|FIDO2ENROLL_UV)) == 0);
        assert((required_extensions & ~FIDO2EXTENSION_HMAC_SECRET) == 0);
        if (FLAGS_SET(required_extensions, FIDO2EXTENSION_HMAC_SECRET))
        ...
        r = verify_features(d, device, !!required_extensions ? LOG_ERR : LOG_DEBUG, FLAGS_SET(required_extensions, FIDO2EXTENSION_HMAC_SECRET), &has_rk, &has_client_pin, &has_up, &has_uv, /* ret_has_always_uv= */ NULL);
        if (r < 0)
                return r;
        ...
        log_info("Generating assertion on FIDO2 security token.");
        ...
        for (;;) {
                ...
                case FIDO_ERR_UP_REQUIRED:
                        ...
                        if (FLAGS_SET(lock_with, FIDO2ENROLL_UP_IF_NEEDED)) {
                                log_notice("%s%sPlease confirm presence on security to unlock.",
                                           emoji_enabled() ? glyph(GLYPH_TOUCH) : "",
                                           emoji_enabled() ? " " : "");
                                plymouth_start_interaction("Please confirm presence on security token to unlock.", &plymouth_displayed);
                ...
                case FIDO_ERR_UNSUPPORTED_OPTION:
                        ...
                        if (has_up && (lock_with & (FIDO2ENROLL_UP|FIDO2ENROLL_UP_IF_NEEDED)) == FIDO2ENROLL_UP_IF_NEEDED) {
                                log_notice("%s%sGot unsupported option error when user presence test is turned off. Trying with user presence test turned on.",
                                           emoji_enabled() ? glyph(GLYPH_TOUCH) : "",
                                           emoji_enabled() ? " " : "");
                                retry_with_up = true;
                ...
                case FIDO_ERR_PIN_REQUIRED:
                        ...
                        if (FLAGS_SET(lock_with, FIDO2ENROLL_PIN) && !strv_isempty(pins))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Token asks for PIN but one was already supplied.");

                        if ((lock_with & (FIDO2ENROLL_PIN|FIDO2ENROLL_PIN_IF_NEEDED)) == FIDO2ENROLL_PIN_IF_NEEDED) {
                                /* If a PIN so far wasn't specified but is requested by the device, and
                                 * FIDO2ENROLL_PIN_IF_NEEDED is set, then provide it */
                                log_debug("Retrying to create credential with PIN.");
                                retry_with_pin = true;
...
int fido2_generate_assert(
        ...
        if (device) {
                r = fido2_is_cred_in_specific_token(device, rp_id, cid, cid_size, FLAGS_SET(required_extensions, FIDO2EXTENSION_HMAC_SECRET), required);
                if (r == 0)
                ...
                return fido2_generate_assert_specific_token(device, rp_id, salt, salt_size, cid, cid_size, pins, required, required_extensions, ret_assert);
        }
        ...
        for (size_t i = 0; i < found; i++) {
                ...
                r = fido2_generate_assert_specific_token(device, rp_id, salt, salt_size, cid, cid_size, pins, required, required_extensions, ret_assert);
                if (!IN_SET(r,
...
int fido2_use_hmac_hash(
        ...
        if (device) {
                r = fido2_is_cred_in_specific_token(device, rp_id, cid, cid_size, /*require_fido2= */ true, required);
                if (r == 0)
                        /* The caller is expected to attempt other key slots in this case,
                         * therefore, do not spam the console with error logs here. */
                        return log_debug_errno(SYNTHETIC_ERRNO(EBADSLT),
        ...
        for (size_t i = 0; i < found; i++) {
                ...
                r = fido2_is_cred_in_specific_token(path, rp_id, cid, cid_size, /*require_fido2= */ false, required);
                if (r < 0) {
                        log_error_errno(r, "Token returned error during pre-flight: %m");
                        goto finish;
                }
...
int fido2_generate_hmac_hash(
        ...
        Fido2ExtensionFlags required_extensions = FIDO2EXTENSION_HMAC_SECRET;
        r = fido2_generate_credential(device, rp_id, rp_name, user_id, user_id_len, user_name, user_display_name, user_icon, askpw_icon, askpw_credential, lock_with, cred_alg, required_extensions, &c, &used_pin, &lock_with);
        if (r < 0)
                return r;
        ...
        if (used_pin) {
                pins = strv_new(used_pin);
                if (!pins)
                        return log_oom();
        }

        r = fido2_generate_assert_specific_token(device, rp_id, salt->iov_base, salt->iov_len, cid, cid_size, pins, lock_with, required_extensions, &a);
        if (r < 0)
                return r;
