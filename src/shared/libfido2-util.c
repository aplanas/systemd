...
static int fido2_generate_credential(
                const char *device,
                const char *rp_id,
                const char *rp_name,
        ...
        r = verify_features(d, device, !!required_extensions ? LOG_ERR : LOG_DEBUG, /* require_hmac_secret= */ true, &has_rk, &has_client_pin, &has_up, &has_uv, &has_always_uv);
        if (!!required_extensions && r < 0)
                return r;
...
static int fido2_generate_assert(
                const char *device,
                const char *rp_id,
                const char *rp_name,
                const void *user_id, size_t user_id_len,
        ...
        r = verify_features(d, device, !!required_extensions ? LOG_ERR : LOG_DEBUG, /* require_hmac_secret= */ true, &has_rk, &has_client_pin, &has_up, &has_uv, /* ret_has_always_uv= */ NULL);
        if (!!required_extensions && r < 0)
                return r;
        ...
        const uint8_t *cd_hash, size_t cd_hash_size;

        r = fido2_assert_set_basic_properties(a, rp_id, cd, cd_size, cid, cid_size);
        if (r < 0)
                return r;
...
int fido2_play(const char *device,
               ...
               char **ret_usedpin,
               Fido2EnrollFlags *ret_locked_with) {
        printf("XXXX FIDO2 Playroom\n");
        int r;
        _cleanup_(fido_cred_free_wrapper) fido_cred_t *c = NULL;
        _cleanup_(fido_assert_free_wrapper) fido_assert_t *a = NULL;

        printf("Generating the credential.. %d\n", lock_with);
        r = fido2_generate_credential(device, rp_id, rp_name, user_id, user_id_len, user_name, user_display_name,
                                      user_icon, askpw_icon, askpw_credential, lock_with, cred_alg, 0, &c, NULL, &lock_with);
        if (r < 0)
                printf("XXXX Error generating the credential\n");

        printf("cred_id..\n");
        const void *cid = sym_fido_cred_id_ptr(c);
        if (!cid)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to get FIDO2 credential ID.");

        printf("cred_size..\n");
        size_t cid_size = sym_fido_cred_id_len(c);

        printf("XXX Checking the credential.. %d\n", lock_with);
        r = fido2_is_cred_in_specific_token(device, rp_id, cid, cid_size, /*require_fido2= */ false, lock_with);
        if (r == 0)
                printf("The credential is not in the token %s.", device);
        if (r < 0)
                return log_error_errno(r, "Token returned error during pre-flight: %m");

        printf("XXX Generating assert..\n");
        uint8_t cd[4] = {0, 0, 0, 0};
        r = fido2_generate_assert(device, rp_id, rp_name, user_id, user_id_len, user_name, user_display_name,
                                  user_icon, askpw_icon, askpw_credential, lock_with, cred_alg, cd, 4, &a);
        if (r < 0)
                return log_error_errno(r, "Token returned error generating the assert: %m");
        return -1;
}
